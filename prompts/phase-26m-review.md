# Phase 26M Review Prompt: Coordinate-anchor translation helper with symbolic and work-plane-local support
...
Phase 26M should add pure coordinate-anchor translation helpers.

## Review checklist

Check:

- helper translates coordinate anchors only;
- layer-bound referencing geometry not directly mutated;
- global symbolic expressions preserved;
- work-plane-local frame origin moves;
- local `a,b` unchanged;
- basis vectors unchanged;
- internal coordinateRef dependencies detach first;
- previews recompute;
- operation atomic;
- invalid/non-finite cases fail safely;
- active work plane not mutated.

Medium issues include:

- refs in layer-bound objects detached during coordinate translation;
- work-plane-local expressions expanded/lost;
- stale previews remain;
- partial mutation on failure;
- selected dependent coordinates move inconsistently.
