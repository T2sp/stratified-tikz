# Phase 16C Review Prompt: Layer duplicate and layer delete
...
Phase 16C should implement duplicating and deleting entire layers with their elements.

## Review checklist

Check duplicate:

- all strata on source layer copied;
- labels copied;
- copied IDs are new;
- copied layer value is target layer;
- source layer unchanged;
- geometry deep-copied;
- styles preserved;
- path label collision policy safe;
- metadata for target layer created;
- SVG/TikZ update.

Check delete:

- all strata on layer removed;
- labels removed;
- metadata removed/updated;
- other layers unchanged;
- selection/filter/drafts validated;
- undo/redo works;
- save/load unaffected.

Medium issues include:

- duplicate reuses IDs;
- duplicate misses labels or geometry kinds;
- delete leaves stale selection causing crashes;
- undo cannot restore deletion;
- TikZ/SVG inconsistent after operation.
