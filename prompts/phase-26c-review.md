# Phase 26C Review Prompt: Coordinate references in path/sheet/label/point inputs and TikZ output
...
Phase 26C should allow elements to reference coordinate anchors and export `(A)` references.

## Review checklist

Check:

- coordinateRef source exists;
- supported fields clear;
- coordinate refs save/load;
- preview resolves refs;
- moving coordinate updates refs;
- missing refs rejected;
- TikZ uses `(tikzName)`;
- coordinate definitions before references;
- inline no blank lines;
- existing direct/numeric fields unaffected.

Medium issues include:

- coordinate refs copied to numeric values;
- dangling refs accepted;
- refs exported before definitions;
- moving coordinate does not update preview;
- unsupported fields silently accept refs.
