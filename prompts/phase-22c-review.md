# Phase 22C Review Prompt: 2D path intersection detection for string diagrams
...
Phase 22C should detect 2D path intersections and expose crossing candidates for later braiding.

## Review checklist

Check:

- 2D-only intersection detection;
- distinct path intersections detected;
- cubic/arc/template sampling works where claimed;
- shared endpoint/overlap policy clear;
- candidates finite and deterministic enough;
- SVG markers preview-only;
- 3D disabled;
- no TikZ export changes yet;
- existing path behavior not regressed.

Medium issues include:

- false intersections on shared endpoints;
- NaN candidates;
- 3D crossings shown;
- markers exported to TikZ;
- path sampling unbounded.
