# Phase 17A Review Prompt: User-editable structured style presets
...
Phase 17A should make structured style presets user-editable and export user-created local styles as `\begin{tikzpicture}` options, not as pre-picture `\tikzset`.

## Review checklist

Check:

- user presets can be created from current style;
- user presets can be renamed;
- user presets can be edited;
- user presets can be deleted;
- built-in presets remain available and are not accidentally overwritten;
- compatible presets can be applied to selected elements;
- incompatible presets are hidden or rejected;
- SVG preview updates after preset application;
- undo/redo behavior consistent with style editing;
- user presets save/load;
- old diagrams without presets load;
- TikZ style names sanitized/collision-safe;
- user preset definitions appear inside `\begin{tikzpicture}[...]`;
- no pre-picture `\tikzset{...}` is emitted for Phase 17A presets;
- layer-aware output preserved.

Medium issues include:

- user presets not persisted;
- local styles emitted as pre-picture `\tikzset`;
- broken TikZ syntax in `tikzpicture` options;
