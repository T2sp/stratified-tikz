# Phase 15C Review Prompt: SVG and TikZ fill output with even-odd rule
...
Phase 15C should render/export 2D and 3D filled closed-boundary objects, including multiple boundaries with `evenOdd` fill rule.

## Review checklist

Check:

- 2D filled regions render in SVG;
- 3D work-plane-filled sheets render in SVG;
- SVG uses `fill-rule="evenodd"` for evenOdd;
- TikZ uses `even odd rule` for evenOdd;
- multiple boundaries exported;
- line and cubic segments exported correctly;
- 3D work-plane-local export uses `canvas is plane` when appropriate or has valid fallback;
- style/layer preserved;
- selected highlight works;
- no source references exported;
- no NaN/infinite coordinates;
- existing paths/sheets not regressed.

Medium issues include:

- evenOdd ignored;
- multiple boundaries flattened incorrectly;
- 3D fill exported invalidly;
- style/layer lost;
- filled object invisible.
