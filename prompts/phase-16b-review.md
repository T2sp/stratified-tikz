# Phase 16B Review Prompt: Layer rename and layer swap/reorder
...
Phase 16B should implement layer rename and layer swap/reorder behavior.

## Review checklist

Check:

- layer rename UI exists;
- renaming changes metadata name, not element membership;
- blank names handled safely;
- renamed layers save/load correctly;
- swap operation updates all strata on the two layers;
- swap operation updates all labels on the two layers;
- no other layers changed;
- metadata remains coherent after swap;
- layer filter/selection validated after swap;
- undo/redo treats swap/rename as expected;
- TikZ output reflects swapped membership;
- layer list ordering deterministic.

Medium issues include:

- swap misses labels or some stratum kinds;
- rename breaks save/load;
- swap changes geometry unexpectedly;
- TikZ output inconsistent after swap.
