# Phase 26E Review Prompt: Coordinate anchors integration with editing, snapping, selection, and layer operations
...
Phase 26E should integrate coordinate anchors with existing editing systems.

## Review checklist

Check:

- snap behavior matches policy;
- direct symbolic edits not snapped;
- coordinate selection cleaned;
- layer filters do not hide shown coordinates;
- layer operations do not treat coordinates as layer-bound;
- path concatenation handles coordinate refs/previews;
- New layer does not affect coordinates;
- TikZ output unaffected by selection/UI state.

Medium issues include:

- coordinates deleted/moved by layer operations;
- coordinate refs lost in path concatenation;
- layer filter hides coordinates unexpectedly;
- direct symbolic coordinates snapped;
- selection saved/exported.
