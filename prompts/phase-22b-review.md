# Phase 22B Review Prompt: Arrow UI, SVG preview, and path direction reversal
...
Phase 22B should add arrow controls, SVG preview, and path direction reversal.

## Review checklist

Check:

- arrow controls appear for path-like objects;
- endpoint/mid arrow options editable;
- arrow head choice editable;
- default mid position is 0.5;
- SVG arrowheads render finite in 2D/3D;
- path reversal works for supported path kinds;
- reversal preserves style/layer/name/id;
- undo/redo works;
- unsupported path kinds handled safely;
- TikZ updates;
- no geometry/TikZ regressions for paths without arrows.

Medium issues include:

- reverse path mutates source incorrectly;
- arrow preview creates NaN;
- TikZ arrow options not updated;
- unsupported path reversal corrupts path;
- 3D path arrows broken.
