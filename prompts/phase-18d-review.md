# Phase 18D Review Prompt: Export mode polish, documentation, and regression coverage
...
Phase 18D should polish the standalone/inline math export mode feature and document it clearly.

## Review checklist

Check:

- export mode UI labels clear;
- help text or docs explain inline math mode;
- docs include align example;
- copy/download uses selected mode;
- optional filenames clear if implemented;
- representative regression tests added;
- standalone output still works;
- inline output has baseline;
- inline output has no blank lines;
- inline setup is inside picture;
- inline external style handling uses comments only;
- no imported `\tikzset` inlined;
- no active `\input` emitted by default.

Medium issues include:

- docs contradict behavior;
- copy/download ignores selected mode;
- inline mode not safe for align;
- standalone regressed.
