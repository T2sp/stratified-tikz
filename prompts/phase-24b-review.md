# Phase 24B Review Prompt: Multi-selection state and selection UI
...
Phase 24B should introduce multi-selection state and UI, without bulk editing yet.

## Review checklist

Check:

- multi-selection state exists;
- Shift/modifier-click toggles selection;
- same-geometric-kind policy enforced/documented;
- background click clears;
- stale ids cleaned after delete/load/layer delete;
- Inspector summary shown;
- multiple objects highlighted;
- selection not saved/exported;
- existing selection/drag/crossing behavior not regressed.

Medium issues include:

- invalid ids remain selected;
- mixed-kind selection silently breaks Inspector;
- selection stored in Diagram;
- crossing marker clicks broken;
- geometry handle drag broken.
