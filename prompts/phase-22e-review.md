# Phase 22E Review Prompt: TikZ/SVG braiding rendering without knot package
...
Phase 22E should render/export braidings using an explicit gap/mask approach without TikZ knot package.

## Review checklist

Check:

- no braiding draws normally;
- braiding/anti determine over/under correctly;
- SVG mask/over redraw works;
- TikZ emits mask/over commands;
- no knot package/library used;
- arrow decorations on main paths preserved;
- no duplicate arrowheads on crossing redraw if policy says so;
- finite coordinates;
- inline no blank lines;
- 4-space indentation;
- 3D braiding disabled/ignored.

Medium issues include:

- knot package used;
- braiding/anti convention reversed or undocumented;
- mask commands erase wrong strand;
- TikZ output malformed with arrows;
- crossing output creates NaN.
