# Phase 15A Review Prompt: Closed-boundary fill data model
...
Phase 15A should introduce robust data models and validation for:

- 2D codim-0 filled regions;
- 3D codim-1 work-plane-local filled sheets;
- multiple closed boundaries with an `evenOdd` fill rule.

## Review checklist

Check:

- 2D filled region has `geometricKind: "region"` and codim 0;
- 3D filled sheet has `geometricKind: "sheet"` and codim 1;
- boundaries are closed paths;
- multiple boundaries supported;
- `fillRule` supports `evenOdd`;
- invalid fill rules rejected;
- open boundaries rejected;
- adjacent endpoint mismatches rejected;
- non-finite coordinates rejected;
- 2D z handling correct;
- 3D planarity/work-plane validation correct;
- style/layer/name handled;
...
- evenOdd not represented;
- non-planar 3D boundary accepted;
- live path references stored instead of copied geometry.
