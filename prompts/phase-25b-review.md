# Phase 25B Review Prompt: Direct input and Inspector UI for work-plane-local symbolic coordinates
...
Phase 25B should expose work-plane-local symbolic coordinate input in direct input and Inspector UI.

## Review checklist

Check:

- coordinate mode UI exists in 3D direct forms;
- local `a,b` accept symbolic expressions;
- active frame snapshot stored;
- preview values displayed;
- Inspector shows/edits local expressions;
- invalid expressions rejected;
- snap does not affect direct symbolic input;
- existing global direct input unaffected;
- supported fields documented;
- save/load works.

Medium issues include:

- local input converted to global numeric only;
- active work plane changes existing object;
- direct input snaps symbolic expressions;
- Inspector edits corrupt source;
- unsupported fields fail silently.
