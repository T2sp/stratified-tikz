# Phase 26A Review Prompt: Coordinate anchor model, save/load, and global TikZ export
...
Phase 26A should add the coordinate anchor model, validation, save/load, and global `\coordinate` export.

## Review checklist

Check:

- coordinate anchors are distinct from point strata;
- no layer/codim/style attached;
- old diagrams load;
- id/name/tikzName validation works;
- global and local position sources validate;
- save/load round-trips;
- TikZ emits `\coordinate` before drawing commands;
- coordinates are not in `pgfonlayer`;
- inline output no blank lines;
- View/New layer state does not affect coordinate export.

Medium issues include:

- coordinate anchors stored as point strata;
- duplicate TikZ names allowed;
- coordinates exported after use;
- coordinates hidden by layer logic;
