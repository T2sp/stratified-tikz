# Phase 27A Review Prompt: Selection cycling for overlapping preview objects
...
Phase 27A should add selection cycling for overlapping objects.

## Review checklist

Check:

- overlapping candidates can be collected;
- normal click behavior preserved;
- cycle modifier works and wraps;
- candidate order deterministic;
- hidden/filtered objects excluded;
- coordinate show/hide respected;
- geometry handles and braiding interactions not broken;
- cycling state UI-only;
- TikZ/export unaffected.

Medium issues include:

- cycling mutates diagram;
- hidden objects selectable;
- normal selection priority regressed;
- braiding marker clicks broken;
- candidate collection unbounded/slow.
