# Phase 26O Review Prompt: Drag translation for multi-selected coordinate anchors with snap support
...
Phase 26O should add drag translation for coordinate multi-selection.

## Review checklist

Check:

- dragging selected coordinate moves all selected coordinates;
- non-selected coordinates unaffected;
- refs remain live;
- referencing geometry preview follows;
- snap applies to drag;
- Inspector numeric translation not affected;
- history has one entry;
- undo/redo works;
- hidden coordinates not draggable;
- single-coordinate drag still works.

Medium issues include:

- references detached during coordinate drag;
- non-selected coordinates move;
- snap creates jitter/stale preview;
- many history entries during drag;
- geometry handles regressed.
