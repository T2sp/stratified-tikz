# Phase 20E Review Prompt: Surface face depth sorting
...
Phase 20E should implement optional approximate surface face depth sorting.

## Review checklist

Check:

- visibility/depth sort option exists;
- default behavior clear;
- surface faces sorted by layer/depth/original index;
- SVG sorted rendering works when enabled;
- TikZ sorted output works when enabled;
- disabled mode preserves prior output;
- inline no blank lines preserved;
- painter's algorithm limitations documented.

Medium issues include:

- sort always changes output unexpectedly;
- layer order ignored unexpectedly;
- non-finite depths exported;
- unstable sorting causes nondeterministic output.
