# Phase 16D Review Prompt: Layer translation
...
Phase 16D should translate all elements on a layer while preserving relative positions.

## Review checklist

Check:

- UI exists for layer translation;
- 2D uses dx/dy and keeps z=0;
- 3D uses dx/dy/dz;
- finite validation works;
- points translated;
- labels translated;
- curves/paths translated;
- arc/circle/ellipse templates translated;
- sheets translated;
- filled regions/sheets translated;
- curved surface primitives translated if present;
- frame origins translated and basis vectors unchanged;
- elements on other layers unchanged;
- ids/styles/layer values preserved;
- undo/redo one-step behavior works;
- SVG/TikZ update.

Medium issues include:

- missing major element kinds;
- translating frame basis vectors incorrectly;
- 2D z becomes nonzero;
