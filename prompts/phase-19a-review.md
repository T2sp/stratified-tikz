# Phase 19A Review Prompt: Symbolic scalar expression model and evaluator
...
Phase 19A should introduce a safe symbolic scalar expression model, validator, evaluator, and TikZ formatter.

## Review checklist

Check:

- expression model supports numeric and symbolic values;
- parser supports variables, arithmetic, parentheses, elementary functions;
- degree trig semantics match PGFMath;
- evaluator uses variable preview values;
- non-finite results rejected;
- unknown variables/functions rejected;
- dangerous TeX/raw commands rejected;
- no JavaScript eval;
- no TeX execution;
- TikZ formatter maps variables to macros;
- existing numeric coordinate behavior unchanged;
- tests cover precedence, trig, invalid inputs, and TikZ formatting.

Medium issues include:

- use of `eval`;
- backslash/raw TeX accepted in expressions;
- radian trig mismatch without documentation;
- non-finite preview values accepted;
- unknown variables silently treated as zero.
