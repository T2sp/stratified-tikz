# Phase 16E Review Prompt: Layer visibility, locking, and filter integration
...
Phase 16E should add layer visibility and optionally locking controls.
...
- toggle exists;
- hidden layers not rendered in SVG;
- hidden layers not selectable;
- hidden elements remain in diagram data;
- TikZ export policy clear and implemented;
- selection cleared/validated when hidden;
- save/load behavior clear.

Check locking if implemented:

- locked layers visible;
- locked layers not selectable/editable;
- unlock restores editability.

Check integration:

- layer filter combines with visibility;
- creation layer behavior safe;
- undo/redo policy clear.

Medium issues include:

- hidden layers still selectable;
- hidden elements deleted accidentally;
- TikZ output policy surprising/undocumented;
- lock state breaks editing globally.
