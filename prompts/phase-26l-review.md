# Phase 26L Review Prompt: Coordinate-anchor multi-selection state and UI
...
Phase 26L should add coordinate-anchor-only multi-selection without translation yet.

## Review checklist

Check:

- multiple coordinate anchors can be selected;
- Shift/modifier-click toggle works;
- coordinate-only policy enforced;
- mixed coordinate + layer-bound selection not silently allowed;
- Inspector summary shown;
- multiple coordinate markers highlighted;
- hidden coordinates not hit-testable;
- hide coordinates clears/handles selection safely;
- selection not saved/exported;
- stale ids cleaned after delete/load.

Medium issues include:

- mixed selection breaks Inspector;
- hidden coordinates still selectable;
- selection saved into Diagram;
- stale coordinate ids remain selected;
- existing layer-bound selection behavior regressed.
