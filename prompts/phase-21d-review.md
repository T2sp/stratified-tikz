# Phase 21D Review Prompt: Inspector drawer from SVG Preview button
...
Phase 21D should make Inspector open via a Preview upper-right button as a right-side drawer.

## Review checklist

Check:

- Inspector button visible in Preview upper-right;
- Inspector hidden/collapsed until opened;
- drawer opens on Preview right side;
- drawer height matches Preview;
- drawer scrolls internally;
- close behavior works;
- existing inspector editing works;
- drawer clicks do not trigger canvas;
- open state not stored in Diagram;
- TikZ/SVG output unaffected.

Medium issues include:

- Inspector always pushes layout;
- Inspector editing broken;
- overlay blocks canvas unexpectedly;
- drawer state affects saved diagram/TikZ.
