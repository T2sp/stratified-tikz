# Phase 18C Review Prompt: Blank-line-free inline formatter with comment separators
...
Phase 18C should ensure inline math TikZ output contains no blank lines, using comments for readability.

## Review checklist

Check inline output:

- no blank lines matching `^\s*$`;
- no leading blank line;
- no trailing blank line;
- no active setup outside picture;
- baseline still present;
- comments used for readability;
- external style comments do not introduce blank lines;
- output safe for `align`;
- standalone output not unintentionally damaged.

Medium issues include:

- any blank line remains in inline output;
- formatting strips required commands;
- baseline removed;
- output becomes unreadable with no comments;
