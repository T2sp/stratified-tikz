# Phase 19C Review Prompt: Symbolic coordinate input in inspector and direct creation
...
Phase 19C should allow coordinate fields to accept symbolic expressions using variables and elementary functions while preserving numeric SVG preview.

## Review checklist

Check:

- coordinate components can store symbolic expression plus numeric preview;
- variables/functions parse and evaluate;
- unknown variables rejected;
- non-finite previews rejected;
- TikZ export uses macro expressions, not preview numbers;
- example `R*cos(q), R*sin(q)` exports with `\R` and `\q`;
- variable changes update preview;
- 2D z remains 0;
- direct creation supports symbolic input for required fields;
- save/load preserves expressions;
- numeric behavior unchanged;
- inline output has no blank lines.

Medium issues include:

- symbolic expression exported as preview number;
- SVG preview cannot render symbolic coordinates;
- unknown variables accepted;
- 2D z becomes symbolic/nonzero;
- numeric input regressed.
