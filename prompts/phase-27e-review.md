# Phase 27E Review Prompt: UI polish — Layer Actions translucency, lenient Inspector numeric inputs, and Add path naming
...
Phase 27E should polish Layer Actions, Inspector numeric inputs, and Add path naming.

## Review checklist

Check:

- Actions panel semi-transparent but usable;
- layer actions still work;
- Inspector numeric fields allow intermediate invalid text;
- `.5` easy to type and commits 0.5;
- invalid text shows warning and does not mutate diagram;
- no NaN geometry;
- Add path label changed to Arbitrary path;
- tooltip/help clarifies line/arc/Bézier mix;
- creation behavior unchanged.

Medium issues include:

- invalid inspector text commits NaN;
- numeric inputs still block `.5`;
- layer actions unreadable/unusable;
- Add path handler broken;
- direct input regressed.
