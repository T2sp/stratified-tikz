# Review Phase 9C: Layer-based selection and filtering

Do not modify files yet. Review the current implementation of Phase 9C only.
...
Phase 9C review scope:
- Verify layer filter is editor-only UI state and not stored in `Diagram`.
- Verify available layer options include strata and labels, sorted numerically, with `All layers`.
- Verify elements outside a selected layer are hidden/de-emphasized and not selectable.
- Verify stale selection is cleared or made safe when the active layer changes.
- Verify labels participate in filtering.
- Verify generated TikZ is unaffected by filtering/selection/highlighting.
- Verify Phase 9A/9B behavior is preserved, including Phase 9B codimension section ordering inside layers.
- Verify no unrelated features were implemented.
...
1. Human-readable review with sections:
   ...
   - Test results
   - Build results
   - Ready to call Phase 9C complete
