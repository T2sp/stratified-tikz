# Phase 19F Review Prompt: Grid TikZ export using foreach and clip
...
Phase 19F should export grid objects compactly using TikZ `\foreach` and rectangular `\clip`.

## Review checklist

Check:

- 2D grid export uses `\foreach`;
- 2D grid export uses `\clip`;
- 3D grid export uses stored work-plane frame and `canvas is plane`;
- grid is not expanded into many draw commands by default;
- style/layer preserved;
- invalid ranges rejected;
- symbolic range policy safe;
- inline output has no blank lines;
- 4-space indentation preserved;
- generated TikZ finite and valid-looking;
- existing output not regressed.

Medium issues include:

- grid expands to huge explicit output;
- broken `\foreach` for symbolic values;
- 3D grid depends on transient active work plane;
- no clip despite requirement;
