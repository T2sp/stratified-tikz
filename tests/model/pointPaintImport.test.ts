...
test('known nested references resolve at their exact position with namespaced lookup', () => {
  const source = String.raw`\tikzstyle{base}=[fill=blue,opacity=.4]
\tikzset{ns/.cd, inner/.style={base,text=red}, outer/.style={draw opacity=.8,inner,fill opacity=.6}}`
